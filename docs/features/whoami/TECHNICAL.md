...
- **`project` is always present in json output**, `null` when the call was unconfined. A server that predates project keys omits the field entirely, which decodes to the same `nil` — clients branch on one condition, not two.
...
- **`status: "ok"` is preserved on output.** The client decodes the wire shape into `*Whoami` (dropping the redundant status flag), but the CLI re-injects `"status":"ok"` into the rendered payload so JSON consumers see the full server contract.
