...
- Under a project key, `--format=text` adds two lines between the agent and the permission list:
  ...
  With `level: none` the access line spells out the situation instead of showing a bare word: *"none — the key is valid but this agent has no access to that project"*. Without a project key the output is unchanged from before project keys existed — no project lines at all.
