...
`internal/client/client.go` exposes `ListChecklistItems`, `CreateChecklistItem`, `SetChecklistItemCompleted`, `UpdateChecklistItem`, `SetChecklistItemState`, `DeleteChecklistItem`, `GetNotes`, `SetNotes`. `ListChecklistItems(ctx, taskID, full)` appends `?full=true` when `full` is set. `internal/cli/checklist.go` wires the `checklist` command tree; the parent `RunE` handles the listing behaviour (and binds the `--full` bool) while `add` / `check` / `uncheck` / `update` / `state` / `pr` / `delete` dispatch as sibling subcommands. `internal/cli/note.go` wires the top-level `note` command. `state` / `pr` are covered in [item-state-and-pr](../item-state-and-pr/TECHNICAL.md), not here.
...
| `internal/cli/checklist.go` | Parent `checklist` command + `add` / `check` / `uncheck` / `update` / `state` / `pr` / `delete` subcommands; `checklistItemMap` / `checklistMaps`. `newChecklistDeleteCmd` / `runChecklistDelete` handle the hard-delete + idempotent-404 path. |
...
- `checklist_item` fields: `id`, `title`, `completed`, `position`, `state`, `pr_number`, `has_notes`, `last_actor`. `state` and `pr_number` are nullable and decode to their zero value (`""` / `0`) rather than a pointer; the renderers map that back to a literal `null`, so both keys are always present in output — see [item-state-and-pr](../item-state-and-pr/TECHNICAL.md). On the `?full=true` read paths each item additionally carries `notes` (a string, `""` when none) — decoded into `ChecklistItem.Notes *string` (`nil` ⇒ field absent on a non-full fetch or a single-item write response). `checklistItemMap` emits the `notes` key only when the pointer is non-nil, so non-full and write responses keep their shape.
