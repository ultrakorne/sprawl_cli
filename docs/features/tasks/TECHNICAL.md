...
`task <id> --full` requests `GET /api/v1/tasks/:id?full=true`, which adds a `checklist_items: [...]` array to the task object (ordered by position), each item being the usual checklist-item shape plus a `notes` string. The decode side adds `Task.ChecklistItems []*ChecklistItem` and `ChecklistItem.Notes *string`, both `omitempty` with a `nil`-vs-present distinction (mirroring `MatchedChecklistItems`): `taskMap` emits `checklist_items` only when the slice is non-nil, and `checklistItemMap` emits `notes` only when the pointer is non-nil — so non-full task / list / search / single-item-write payloads keep their existing shape. The text fallback (`taskDetailText`) appends a `checklist:` block built by `fullChecklistText` (shared with `checklist <id> --full`), indenting items two columns and notes eight. `/tasks/search` responses additionally include `matched_checklist_items: [{id,title}, …]` per task — present (possibly `[]`) only on search payloads, absent everywhere else. The decode side uses `client.MatchedChecklistItem` and a `nil`-vs-empty distinction on `Task.MatchedChecklistItems` so `taskMap` can suppress the key on list/show/create/update output and pass it through verbatim on search output (including the `[]` "matched on title" signal).
