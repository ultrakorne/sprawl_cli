...
### `theme set <id>`
`PATCH /api/v1/settings/theme` body `{"theme": "<id>"}`. Owner-only (non-owner → 403 `forbidden`). **Requires an agent secret** — see below. Unknown / mis-cased id → 404 `theme_not_found` from the server. Missing / blank `theme` key and any other changeset validation failure returns the shared fallback shape `{"errors": {...}}` (no top-level `error` code) — the CLI surfaces this as `error: "invalid"` + `details: <errors>` in json output. Text fallback is `Theme set to <id>`.
