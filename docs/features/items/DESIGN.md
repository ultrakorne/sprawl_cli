...
- `TITLE` — last, unpadded. `queue` adds nothing after it: its rows are grouped under a per-task header (`<id> <title>`, project and due date, description), so the task and project are said once per block instead of once per row.
